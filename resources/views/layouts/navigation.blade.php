<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    {{-- Only ADMIN has admin dashboard --}}
                    {{-- ADMIN MENU --}}
                    ...
                    {{-- Only SELLER (member with store) --}}
                    @if(auth()->check() && auth()->user()->role === 'member' && auth()->user()->store)
                        <x-nav-link :href="route('seller.dashboard')" :active="request()->routeIs('seller.dashboard')">
                            {{ __('Seller Dashboard') }}
                        </x-nav-link>
                    @endif

                    {{-- CUSTOMER tidak punya dashboard → tidak ditampilkan --}}
                </div>
            </div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        ...
        {{-- Customer: no dashboard menu shown --}}
